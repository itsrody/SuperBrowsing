...
(async () => {
    ...
    const createStyles = () => {
      ...
      #${INDICATOR_ID}.seeking .vg-progress-bg {
    ...
    const showIndicator = (iconName, text, type = '', sticky = false, progress = null) => {
        if (!indicator) return;
        ...
        let content = `<div class="vg-row"><span class="vg-icon">${iconSvg}</span><span>${text}</span></div>`;

        if (type === 'seeking' && progress !== null) {
            content += `<div class="vg-progress-bg"><div class="vg-progress-fill" style="width: ${progress}%"></div></div>`;
        }
    ...
    const findVideo = (targetElement, x, y) => {
        try {
            ...
            // Try point search with Shadow DOM support
            if (typeof x === 'number' && typeof y === 'number') {
                ...
                for (const element of elements) {
                    ...
                    if (element.shadowRoot) {
                        ...
                        if (shadowElement) {
                            const shadowVideo = shadowElement.closest('video');
            ...
            // Find largest video (recursive for Shadow DOM)
            let largest = null;
            let maxArea = 0;

            const findAllVideos = (root) => {
    ...
    const onTouchStart = (e) => {
        try {
            ...
            gestureState = {
                ...
                initialBrightness: parseFloat(result.video.dataset.vgBrightness || '1'),
        ...
        try {
            ...
            if (isVertical) {
                if (zoneX < settings.SWIPE_ZONES.VOLUME) {
                    ...
                    // Middle zone - Swipe down to exit fullscreen
                    gestureState.action = 'fullscreen-drag';
                } else {
                    gestureState.action = 'brightness';
                }
            }
            else {
    ...
    const handleSeeking = (dx) => {
        try {
            ...
            if (now - gestureState.lastIndicatorUpdate > settings.INDICATOR_UPDATE_THROTTLE) {
                ...
                showIndicator(icon, formatTime(newTime), 'seeking', false, progress);
    ...
    const handleVolume = (dy) => {
        try {
            if (isIOS()) {
                showToast('volume-mute', 'System Volume Only', 1000);
                return;
            }
    ...
    const handleBrightness = (dy) => {
        try {
            const change = -dy / settings.VOLUME_SENSITIVITY; // Reuse sensitivity for now
            const newBrightness = Math.max(0.1, Math.min(1.0, gestureState.initialBrightness + change));

            gestureState.video.style.filter = `brightness(${newBrightness})`;
            gestureState.video.dataset.vgBrightness = newBrightness;

            const now = Date.now();
            if (now - gestureState.lastIndicatorUpdate > settings.INDICATOR_UPDATE_THROTTLE) {
                const percent = Math.round(newBrightness * 100);
                showIndicator('brightness', `${percent}%`, 'brightness');
                gestureState.lastIndicatorUpdate = now;
            }
        } catch (e) {
            console.warn('[VideoGestures] Brightness error:', e);
        }
    };
    ...
    const handleFullscreenDrag = (dy) => {
        try {
            ...
            } else if (dy < -settings.SWIPE_THRESHOLD) {
                showIndicator('pip', 'Release for PiP', 'aspect');
            } else {
                hideIndicator();
            }
        } catch (e) {
            console.warn('[VideoGestures] Fullscreen drag error:', e);
        }
    };

    const handleSwipeEnd = () => {
        try {
            if (gestureState.action === 'volume' || gestureState.action === 'seeking' || gestureState.action === 'brightness') {
                vibrate();
            } else if (gestureState.action === 'fullscreen-drag') {
                const dy = gestureState.lastY - gestureState.startY;
                if (dy > settings.SWIPE_THRESHOLD * 2) { // Require a bit more distance for exit
                    exitFullscreen();
                } else if (dy < -settings.SWIPE_THRESHOLD * 2) { // Swipe up for PiP
                    togglePip();
                } else {
                    hideIndicator();
                }
            } else if (gestureState.action === 'fullscreen') {
                // Legacy support if needed, or cleanup
                const dy = gestureState.lastY - gestureState.startY;
    ...
    const togglePip = async () => {
        try {
            if (!document.pictureInPictureEnabled || !gestureState?.video) return;

            if (document.pictureInPictureElement) {
                await document.exitPictureInPicture();
                showIndicator('pip', 'Exit PiP');
            } else {
                await gestureState.video.requestPictureInPicture();
                showIndicator('pip', 'Enter PiP');
            }
            vibrate();
        } catch (e) {
            console.warn('[VideoGestures] Toggle PiP error:', e);
            showToast('pip', 'PiP Not Supported');
        }
    };
    ...
    const getIconSvg = (name) => {
        const icons = {
            ...
            'brightness': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M20 15.31L23.31 12 20 8.69V4h-4.69L12 .69 8.69 4H4v4.69L.69 12 4 15.31V20h4.69L12 23.31 15.31 20H20v-4.69zM12 18c-3.31 0-6-2.69-6-6s2.69-6 6-6 6 2.69 6 6-2.69 6-6 6z"/></svg>',
            'pip': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M19 11h-8v6h8v-6zm4 8V4.98C23 3.88 22.1 3 21 3H3c-1.1 0-2 .88-2 1.98V19c0 1.1.9 2 2 2h18c1.1 0 2-.9 2-2zm-2 .02H3V4.97h18v14.05z"/></svg>'
        };
        return icons[name] || '';
    };
    ...
    const init = () => {
        try {
            ...
            const observer = new MutationObserver(() => {
                ...
                debounceTimer = setTimeout(checkForVideos, 1000);
            });
