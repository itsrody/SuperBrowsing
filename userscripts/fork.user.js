...
Memory Usage: ${stats.memoryUsage} cached videos`);
    ...
    async function initialize() {
        try {
            ...
            // Discover existing videos
            document.querySelectorAll('video').forEach(video => {
                videoManager.registerVideo(video);
            });

            if (CONFIG.DEBUG_MODE) {
                console.log('🚀 Video Gestures Pro Enhanced initialized');
                window.VGE = { gestureState, uiManager, videoManager, gestureEngine, CONFIG };
                
                // Add test function
                window.VGE.test = {
                    showIndicator: (text) => uiManager.showIndicator({ type: 'play', text: text || 'Test' }),
                    showAllIcons: () => {
