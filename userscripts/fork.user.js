// ==UserScript==
// @name         Universal Video Touch Gestures (Pro) - Swipe Up Fork
...
(async function() {
    ...
    let config = await GM_getValue('config-fork-swipe-up', DEFAULTS);

    // ** Individual Menu Commands for each setting **
    const menuCommands = {
        MIN_VIDEO_DURATION_SECONDS: '[Fork] Min Video Duration (s)',
        DOUBLE_TAP_SEEK_SECONDS: '[Fork] Double-Tap Seek (s)',
        SWIPE_THRESHOLD: '[Fork] Swipe Threshold (px)',
        SEEK_SENSITIVITY: '[Fork] Seek Sensitivity',
        ENABLE_HAPTIC_FEEDBACK: '[Fork] Toggle Haptic Feedback',
        HAPTIC_FEEDBACK_DURATION_MS: '[Fork] Haptic Duration (ms)',
        FORCE_LANDSCAPE: '[Fork] Toggle Force Landscape'
    };

    // Function to create a menu command for a given setting
    const createMenuCommand = (key, description) => {
        GM_registerMenuCommand(description, async () => {
            const currentValue = config[key];
            let newValueStr = prompt(`Set "${description}":\n(Current value: ${currentValue})`, currentValue);
            ...
            if (newValueStr !== null) {
                ...
                if (typeof defaultValue === 'boolean') {
                    newValue = newValueStr.toLowerCase() === 'true' || newValueStr === '1';
                } else if (typeof defaultValue === 'number') {
                    newValue = parseFloat(newValueStr);
                    if (isNaN(newValue)) {
                        alert('Invalid number. Setting not saved.');
                        return;
                    }
                } else {
                ...
                config[key] = newValue;
                await GM_setValue('config-fork-swipe-up', config);
                alert(`"${description}" set to ${newValue}.\nPlease reload the page for changes to take effect.`);
            }
        });
    };

    for (const key in menuCommands) {
        createMenuCommand(key, menuCommands[key]);
    }


    // --- Styles ---
    function injectStyles() {
        if (document.getElementById('video-gesture-pro-styles-fork')) return;
        const style = document.createElement('style');
        style.id = 'video-gesture-pro-styles-fork';
        style.innerHTML = `
    ...
    function onTouchMove(e) {
        ...
        if (Math.abs(deltaX) > config.SWIPE_THRESHOLD || Math.abs(deltaY) > config.SWIPE_THRESHOLD) {
            if (gestureType === 'tap') {
                ...
                // ** MODIFIED: Check for swipe up in the middle for fullscreen **
    ...
    function onTouchEnd(e) {
        ...
        if (gestureType === 'swipe-y-fullscreen') {
            ...
            // ** MODIFIED: Check for swipe UP (negative deltaY) **
