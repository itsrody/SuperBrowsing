    ...
    */
    ...
    cleanContext(window).then(__CONTEXT__ => {
        ...
        const promiseForTamerTimeout = new Promise(resolve => {
            ...
            !isUrlInEmbed && isAbortSignalSupported && typeof customElements === "object" && (isMobile ? Promise.resolve() : whenCEDefined('ytd-app')).then(() => {
        ...
        FIX_ytAction_ && (async () => {
            ...
            const appTag = isChatRoomURL ? 'yt-live-chat-app' : (isMobile ? 'ytm-app' : 'ytd-app');

            const ytdApp = await new Promise(resolve => {

                (isMobile ? Promise.resolve() : whenCEDefined(appTag)).then(() => {
                    ...
                    if (isMobile && !document.querySelector(appTag)) {
                        resolve(document.body); // Fallback for mobile
                        return;
                    }
                    mo.observe(document, { subtree: true, childList: true });
                });
            ...
            if (!ytdApp) return;
            const cProto = isMobile ? {} : insp(ytdApp).constructor.prototype;
        ...
        */
        const _yt_player_observable = observablePromise(() => {
            const _yt_player = (((window || 0)._yt_player || 0) || 0) || (isMobile ? document.getElementById('movie_player') : 0);
            if (_yt_player) {
        ...
        */
        ...
        (FIX_schedulerInstanceInstance & 2) && (async () => {
            ...
            if (checkOK) {
                ...
                */
                ...
                if (schedulerInstanceInstance_ && schedulerInstanceInstance_.start && schedulerInstanceInstance_.start993) {
        ...
        !isUrlInEmbed && Promise.resolve().then(() => {
            ...
            FIX_avoid_incorrect_video_meta_bool && !isMobile && whenCEDefined('ytd-video-primary-info-renderer').then(() => {
                let dummy;
                let cProto;
                // let mc = 4;
            ...
            FIX_ytdExpander_childrenChanged && !isMobile && !isChatRoomURL && whenCEDefined('ytd-expander').then(() => {
            ...
            FIX_POPUP_UNIQUE_ID && !isMobile && whenCEDefined('ytd-popup-container').then(async () => {
